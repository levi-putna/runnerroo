 ...
 */
async function startPasswordlessSignUp({
  email,
  fullName,
  emailRedirectTo,
}: {
...
/**
 * Completes passwordless registration using the 6-digit code from the sign-up email (same OTP channel as magic link).
 */
...
/**
 * Passwordless sign-up flow: collect details, email OTP, then verify before accessing the app.
 */
export default function SignUpPage() {
  ...
  async function handleSendCode(e: React.FormEvent) {
    ...
    setSendLoading(true)
    setError(null)
    const { error: otpErr } = await startPasswordlessSignUp({
      email,
      fullName,
  ...
  async function handleVerifyCode(e: React.FormEvent) {
    ...
    setError(null)
    const { error: verifyErr } = await verifySignUpOtp({ email, token: otpCode })
    if (verifyErr) {
    ...
    return (
      <Card>
        <CardHeader className="text-center">
          ...
          <CardDescription>
            We sent a verification PIN to <strong>{email}</strong>. Enter it below to activate your account.
          </CardDescription>
        </CardHeader>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>We&apos;ll email you a one-time PIN to verify before you can sign in</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {/* Social auth */}
        ...
        <div className="relative">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-card px-2 text-muted-foreground">or email PIN</span>
          </div>
        </div>

        {/* Sign-up form: name + email, then PIN is sent */}
