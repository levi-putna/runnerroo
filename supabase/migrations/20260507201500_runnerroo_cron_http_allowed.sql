...
begin
  ...
  if v_lc not like 'https://%' and v_lc not like 'http://%' then
    raise exception
      'runnerroo_add_or_replace_cron_job: url must begin with https:// or http:// — pg_net rejects other schemes.';
  end if;
  ...
  if lower(trim(host_match[1])) in ('localhost', '127.0.0.1', '[::1]') then
    raise exception 'runnerroo_add_or_replace_cron_job: hostname must not be localhost — pg_net cannot reach your machine; set WORKFLOW_CRON_PUBLIC_BASE_URL to a deployed origin.';
  end if;
